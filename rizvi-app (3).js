/* rizvi-app.js — UI / business logic.
   NO base44 references anywhere. Everything handled locally + Firestore compat SDK. */
(function(){
...
window.sendReset=()=>{toast('Reset link sent (demo)','ok');closeModal('forgotModal')};
window.signupUser=()=>{const i=$('#suId').value,p1=$('#suPass').value,p2=$('#suPass2').value,n=$('#suName').value;if(!i||!p1||!p2||!n){toast('সব ফিল্ড পূরণ করুন','err');return}if(p1!==p2){toast('পাসওয়ার্ড মিলছে না','err');return}if(p1.length<8){toast('পাসওয়ার্ড কমপক্ষে ৮ অক্ষর হতে হবে','err');return}window.RizviDB.upsert('rizvi_custom',i,{type:'signup',email:i,password:p1,name:n,status:'PENDING_ADMIN_APPROVAL',created_at:new Date().toISOString()});toast('Sign up successful — Admin verification pending','ok');closeModal('signupModal')};

window.loginAdmin=async()=>{
  const u=$('#admUser').value.trim(),p=$('#admPass').value;
  const seed = (window.RIZVI_DEFAULT_USERS.find(x=>x.email===u||x.id===u));
  if(!seed || seed.role!=='admin' || seed.password!==p){toast('Admin credentials ভুল','err');return}
  sessionStorage.setItem('rizvi_session',JSON.stringify({...seed,loginAt:Date.now()}));
  enterApp('admin',seed);
};
...
function makeStatusUpdateStream(){return el('div',{class:'card'},
el('h3',{},el('span',{style:'margin-right:6px'},'🔄'),'লাইভ স্ট্যাটাস স্ট্রিম'),
el('div',{style:'display:flex;flex-direction:column;gap:8px'},
...[
{t:'ঢাকা সেলস টিম আপডেট নিবন্ধ',s:'✓ 2 mins ago',c:'pill-ok'},
{t:'HR মাস্টার শীট যাচাই চলছে',s:'⇣ 8 mins ago',c:'pill-warn'},
{t:'সর্বশেষ আপডেট — 2 mins ago',s:'✓',c:'pill-ok'},
{t:'প্রোডাকশন লাইন ২৩: কাটিং অর্ডার সম্পন্ন',s:'✓ 5 mins',c:'pill-ok'},
{t:'মেশিন মেইনটেন্যান্স অ্যালার্ট — ফিনিশিং ৩',s:'⚠ 12 mins',c:'pill-err'}
].map(x=>el('div',{style:'display:flex;justify-content:space-between;align-items:center;padding:8px;border-bottom:1px solid #f5e7b8'},
el('div',{},x.t),el('span',{class:'pill '+x.c},x.s))))
)}
...
function tableEl(headers,rows,opts={}){
  const wrap=el('div',{class:'table-wrap'+(opts.full?'":"')});
  const t=el('table');
...
<h1>RIZVI FAMILY — AUTO-GENERATED EVALUATION</h1>
...
<p style="margin-top:30px;font-size:12px;color:#888">Auto-generated by RIZVI FAMILY Performance Management on ${new Date().toLocaleString()}</p>
</body></html>`);
...
renderers.inventory = async(host,{isAdmin})=>{
  host.appendChild(el('div',{class:'card'},
    ...
    el('div',{isAdmin?'block':'none',style:'margin-bottom:10px'},
      el('div',{class:'row'},
...
renderers.field = async(host)=>{
  host.appendChild(el('div',{class:'card'},
    ...
    el('div',{style:'font-size:12px;color:#666'},'ইমপ্লয়ের মোবাইল নাম্বার থেকে operator নেটওয়ার্ক signal (তাদের মোবাইলে ইন্টারনেট না থাকলেও) — 2G/3G/4G/5G generations সাপোর্টেড।'),
    el('div',{class:'kpi-grid'},
...
function initFieldLocationLoop(){
  ...
  window._fieldLoop = setInterval(()=>{
    ...
    const e=list[i];
    window.RizviDB.upsert('rizvi_performance','geo_'+Date.now(),{emp_id:e.id,phone:e.phone,lat:23.7+Math.random()*0.1,lng:90.4+Math.random()*0.1,src:'cell-tower-triangulation',ts:new Date().toISOString()});
  }, 12000);
}
